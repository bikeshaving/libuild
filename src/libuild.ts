    ...
    if (workspaceSpec === "workspace:*") {
      ...
      for (const pkgPath of possiblePaths) {
        ...
        if (await fileExists(resolvedPath)) {
          ...
          const pkgJson = JSON.parse(pkgContent); // Let JSON.parse throw on invalid JSON
...
export async function build(cwd: string, save: boolean = false): Promise<{distPkg: PackageJSON, rootPkg: PackageJSON}> {
  ...
  // Discover ambient .d.ts files early so we can add them to exports
  const ambientDtsFiles: string[] = [];
  if (await fileExists(srcDir)) {
