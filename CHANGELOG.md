...
### Added
- **Deno is a test platform.** `libuild test -p deno` runs a suite under Deno, alongside bun, node and the browsers. Deno bridges `node:test` into its own runner, so the bundle and the `@b9g/libuild/test` backend are the node ones; `deno run` would register the tests and never run them, so the runner uses `deno test --allow-all`. Results are read from a JUnit report Deno writes beside the bundle, not from its summary line, which counts a `describe` as one test and its children as steps. Deno reports a `todo` test as skipped, so the test file counts what it registered as todo and the runner tells them apart; passed, failed, todo and skipped read the same on bun, node and deno.
...
- **A bun test file is counted from bun's own summary.** The runner read the first line that looked like ` N pass` or ` N fail` anywhere in a file's output. A test that logged such a line, or ran bun itself and printed the result, was counted from that text instead: a passing file could report `0 passed, 1 failed`, and a failing one could report as passing. It now uses the last summary, which bun always prints at the end.
- **A string `exports` field, or conditions with no `"."` key, builds as the `"."` export.** `"exports": "./src/index.ts"` failed with `Export path '.' must point to a valid entrypoint`. `"exports": {"import": "./src/index.ts"}` built without error but wrote a dist manifest that mixed `"import"` with `"."` keys, which node refuses to load, so the published package could not be imported. Both forms now mean `"."`, as they do in node, and `--save` keeps a string that names a source file as written.
...
### Fixed
- **`--save` no longer writes a `module` field pointing at nothing.** A package with no main entry (several src entries, none named `index` or matching the package name - libuild's own shape since 0.2.20 stopped guessing one) had that absent entry interpolated into the root manifest as `"module": "./dist/undefined.js"`. Anything resolving through it - a bundler, `npm link` against the repo - then failed to find an entry. `main` had the same hole whenever a CJS format was on. The published manifest already guarded this; only the `--save` path did not.
