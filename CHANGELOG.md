...
## [0.2.15] - 2026-08-06 — SKIPPED (never published)

This version was committed but never tagged or published, so `0.2.15` does not exist on npm — the registry goes `0.2.14` → `0.2.16`, and there is no `v0.2.15` git tag or GitHub release. The changes below shipped to users as part of **0.2.16**. Kept under its own heading rather than folded upward so the history reflects what was built when.
